...
2023 年 12 月、株式会社エクストリームへ入社。フロントエンド開発と顧客用クラウド環境の構築・運用の自動化を経験。2025 年 3 月に退社。
...
これまで主にバックエンド開発での API 設計・実装、データベース設計、AWS を活用したインフラ構築・運用に従事してきました。
フロントエンド側は Vue の経験はあるものの実績が少なく、React や TypeScript といったモダンな開発技術スタックに関してもまだまだ経験が足りないと感じています。
将来的にはプロダクトの企画から運用まで全体を見渡すことができるように、バックエンドからフロントエンドまでを一貫して理解できるフルスタックエンジニアとして活躍していきたいと考えています。
また、技術的な成長だけでなく、これまでのプロジェクト進行の経験を活かし、チーム内でのリーダーシップや、顧客との要件調整など、技術面以外でも貢献できるエンジニアとして成長していきたいです。

## 働き方の希望

- フレックス勤務可を希望（コアタイムの有無や時間帯は気にしません）
